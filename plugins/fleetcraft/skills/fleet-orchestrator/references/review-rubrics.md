...
Prose criteria drift when a cheaper model applies them. These anchors don't. Score
against the anchors; never improvise a scale mid-review.
...
- **2 / unproven:** "tests pass" without showing which tests exercise the change, or a
  regression test never shown failing pre-fix. **FAIL.** A test that passes on the
  parent revision has not demonstrated that it protects the claimed regression.
...
- **8 = strong and deliberate:** a stranger would see a coherent, audience-appropriate
  surface with no material craft or trust defect. This is a review anchor, not a model-
  quality benchmark.
...
majority carries; any single lens can issue a protocol VETO on a gate violation (class 7
fabricated proof, class 3 design regression) regardless of average score. Native platform
controls, not this rubric, are technical enforcement.
...
structure. Mixed → write for the furthest reader.
