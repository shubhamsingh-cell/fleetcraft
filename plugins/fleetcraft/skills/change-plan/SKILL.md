...
Honor an explicit user format, length, or word limit before this default structure.
Treat the limit as a hard delivery constraint: reserve a small margin, compress or
combine sections, and use only the plan elements material to the decision. Before
returning, count words with an available local means when a word limit was requested.

When no stricter user format applies, use this concise structure as a menu rather than
a mandatory expansion:
...
Omit or merge headings that have no material content; preserve critical uncertainty,
approval, acceptance, abort, and recovery information in the available space.
...
available executor role (for example `fleetcraft:executor` in the plugin or a manually
installed executor definition) is the delegation target for bounded implementation, per
`fleet-orchestrator`. A plan approved here is **not** authorization to
