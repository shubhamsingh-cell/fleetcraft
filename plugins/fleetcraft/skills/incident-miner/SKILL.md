...
description: >
  ...
  skill", "log this so we don't hit it again". If an installed skill-creation or
  memory-consolidation workflow is available, it can consume the approved draft; this
  skill does not require either dependency.
---
...
- This handles one new candidate on demand; use a separate consolidation workflow only
  when one is available and suitable.
- If an installed skill-creation workflow is available, it may handle approved packaging
  mechanics (frontmatter, scaffolding, naming); otherwise make the approved project-local
  edit using the repository's conventions.
...
- New skill → invoke an installed skill-creation workflow when available; otherwise create
  the approved project-local skill following repository conventions.
