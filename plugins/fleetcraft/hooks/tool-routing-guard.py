...
This layer provides bounded reminders at web-tool choice time. It does not assume
any named provider is installed or available; a reminder must be checked against the
active harness before selecting an alternative.
...
# Optional documentation routing is bounded to the first matching fetch in a
# session. A retry remains available when the suggested capability is absent.
# A denial proves only this hook decision; it does not establish a routing rate
# or the quality of the alternative source. Default mode provides reminders.
def routing_enforced() -> bool:
    """Read the plugin option first, preserving the legacy environment switch."""
    raw = os.environ.get("CLAUDE_PLUGIN_OPTION_ROUTING_ENFORCE")
    if raw is None:
        raw = os.environ.get("CLAUDE_TOOL_ROUTING_ENFORCE", "0")
    return raw.strip().lower() in {"1", "true", "yes", "on"}


ENFORCE = routing_enforced()

MSG_DOCS_DENY = (
    "tool-routing-guard DENIED this WebFetch because optional enforcement is enabled. "
    "Check for an available documentation capability first; if none applies, retry this fetch. "
    "The one-per-session gate then permits it."
)
...
MSG_WEBSEARCH = (
    ...
    "If an available retrieval integration provides structured search or page "
    "content, it can reduce repeated fetches. Check availability before selecting "
    "one; a single built-in lookup remains valid."
)

MSG_DOCS_URL = (
    "tool-routing-guard: this WebFetch targets a documentation URL "
    "(once-per-session reminder). If a documentation integration is available, "
    "it may provide version-aware API material efficiently. Check availability "
    "first; this hook does not install or require a provider."
)

MSG_PAGE_BY_PAGE = (
    "tool-routing-guard: this session has already made 2+ WebFetch calls. If an available "
    "multi-page retrieval capability fits the task, use its documented bounded workflow; otherwise "
    "continue with the separate pages. This reminder does not install or require a provider."
)
