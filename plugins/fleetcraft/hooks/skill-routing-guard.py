...
CATEGORIES = [
    dict(
        ...
        message=(
            ...
            "If available, the duckdb skill (read-only SELECT/DESCRIBE/SUMMARIZE; `duckdb`) "
            "or qsv skill (fast CSV stats/validation; `qsv`) can help; otherwise use an "
            "available local approach. Prefer bounded read-only analysis for local tabular files."
        ),
    ),
    dict(
        id="doc-extract",
    ...
    dict(
        ...
        message=(
            ...
            "panel (mechanism + trust-and-proof where applicable) -> verdict; an errored lens is "
            "an OPEN finding, never a pass. Invoke the available design review workflow when it fits; "
            "use its checklist as prompts for inspection, not automatic vetoes."
        ),
    ),
    dict(
        id="register-exec",
        ...
        message=(
            ...
            "Register rule: the reader's DISTANCE from the work "
    ...
    dict(
        ...
        message=(
            ...
            "If a context7 MCP or other documentation integration is available, it may provide current, version-aware "
            "material. Check tool availability before selecting it; otherwise use an appropriate "
            "local or web retrieval path and cite only what was observed."
        ),
    ),
    dict(
        id="graphify-first",
        ...
        message=(
            ...
            "If the graph capability is available and fresh, query it before broad grep+read "
            "sweeps for dependency questions. Check freshness first: git log -1 vs built_at_commit in "
    ...
    dict(
        ...
        message=(
            ...
            "An incident-mining workflow can gather corroborating evidence from supplied session "
            "artifacts and any available read-only sources, "
