    ...
    for e in turn:
        ...
        for b in blocks(e.get("message") or {}):
            ...
            if btype == "text":
                ...
                name = b.get("name", "") or ""
                # Delegating to an agent only proves that work was requested.
                # It does not prove that a retrieval attempt occurred, and a
                # parent transcript cannot safely infer the child's tool use.
                if RETRIEVAL_TOOL_RE.search(name):
                    used_retrieval = True
                else:
                    try:
