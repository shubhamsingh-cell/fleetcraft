...
- Run `EXPLAIN (ANALYZE, BUFFERS)` and read it before proposing an index. The plan *is* the
  measurement; "probably needs an index" is a guess with a confident tone.
...
Building an index to see if it helps is a migration you then have to undo. Test it hypothetically
first — the whole loop is read-only:

1. Rank candidates from `pg_stat_statements` (`total_exec_time`, then `mean_exec_time`).
2. `CREATE EXTENSION hypopg;` then `SELECT hypopg_create_index('CREATE INDEX ...')` per candidate.
3. Re-run `EXPLAIN` — **without** `ANALYZE`; hypothetical indexes affect planning only. Record the
   before/after estimated cost. That delta is your evidence for or against.
4. `SELECT hypopg_relation_size(indexrelid)` for the write/storage side of the trade.
5. `SELECT hypopg_reset()` in a `finally` block so no session leaks hypothetical state.

Build only the index whose measured plan-cost delta justifies its write cost.
...
- **⚠ The scale-to-zero trap.** The managed Postgres provider **wipes `pg_stat_statements`
  whenever the compute suspends or scales to zero.** This account's projects were deliberately
  set to 0.25↔1 CU with suspend enabled after the 2026-07 cost incident, so a cold read will
  rank a near-empty table and you will "optimise" nothing. `pg_stat_statements_reset()`, then
  measure across a representative warm window, and say which window you measured.
- Both `hypopg` and `pg_stat_statements` are on the managed Postgres provider's
  supported-extension list (checked 2026-09-07).
...
- **Never cache** balances, permissions/entitlements, or checkout inventory — anything where
  a stale read is a wrong answer rather than an old one.
- **Never cache a failure.** A cache placed in front of an operation that can time out will
  faithfully serve the timeout. (Precedent, a production CRM's facets, 2026-07-31.)
