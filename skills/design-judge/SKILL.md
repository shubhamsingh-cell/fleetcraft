...
name: design-judge
description: >-
  Screenshot-first design judgment protocol — the proven substitute for Fable-tier
  design taste. Load when judging whether a UI/page/deck/creative is ready to ship
  ...
  live. Runs a render→lens-panel→verdict loop with hard vetoes for fabricated proof
  and design regressions. Invoke /design-judge explicitly before shipping any visual
  surface if it hasn't fired on its own.
---

# design-judge — how design taste gets decided without Fable
...
Codifies the protocol that produced real wins across several products: a marketing
site scoring 3/3 Opus judges at "expensive-feel" 8/10, a repeated-motif kill on a
product hub, a deck-template mechanism replacement, and an overturned first diagnosis
on a homepage redesign. Sonnet running THIS protocol replaces a senior design eye;
Sonnet eyeballing a diff does not.
...
1. **Nothing is judged unrendered.** Render the artifact and screenshot it — the diff
   ...
   the code.
2. **Mechanism before surface** (fable-judgment §2 — keep in sync if §2 moves). First
...
- **Context-light screenshot path (added 2026-08-20).** When a lens or re-verify pass
  needs ONLY pixels — no DOM reads, no interaction — `npx playwright-cli` can take the
  screenshot without pushing page state into context (both the in-app browser and the
  Chrome extension do), which matters on long judging sessions where context bloat is
...
- **expensive-feel** — would a stranger assume a serious team built this? (8/10 anchor:
  a marketing site that scored 3/3 Opus judges at this level.)
...
`~/.claude/skills/fleet-orchestrator/references/review-rubrics.md`, the ONE strongest
strength, the ONE change that would raise the score most, and any veto.

Each lens greps its section of `references/ai-slop-checklist.md` (87 named AI-design
