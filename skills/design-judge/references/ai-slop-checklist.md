...
the render→panel→verdict loop and the two hard vetoes (iron rules 2 & 3) still govern. A tell
...
**Provenance:** rule content adapted 2026-08-20 from `miqdadbadjuber/anti-slop` (MIT,
© 2026 Miqdad Badjuber), commit `cd9aeb191d`, read read-only via `gh api` — 87 checks distilled,
its meta-framework (tiers/dials/install-wizard) deliberately NOT adopted (our loop is stronger).
Attribute the source if any of this ships externally. 76 of 87 were new granularity; the 11
`[overlap]` items sharpen vetoes we already run.
...
- Body/label text below WCAG AA: normal < 4.5:1, large (18px+) < 3:1 — measured across the
  *whole* area the text crosses, at the worst spot, not the brightest.
...
> Pair with `~/.claude/skills/organic-growth-engine/references/voice-and-false-positives.md`
> before flagging prose: it lists what NOT to flag (polish, lone em dashes, formal vocabulary,
...
- Visual clone of a named product (Linear/Vercel/Stripe/Notion) with no request to — an originality/trust
  issue (NEW: brand-snap checks fidelity to *your* brand, not non-imitation of *others'*).
