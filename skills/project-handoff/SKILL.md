...
description: "Write or resume the durable state record for long-running work — objective, status, blockers, next actions, decisions and rejected alternatives — so the next session continues instead of re-deriving. Use when resuming a project after a break or compaction, when work will outlive this session, when the user says 'pick up where we left off', 'hand this off', 'write this down before you forget', or at the end of a multi-session push. Not for one-off tasks, chat summaries, status updates to people, or incident lessons (use incident-miner)."
---
