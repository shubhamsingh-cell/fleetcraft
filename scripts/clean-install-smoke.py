...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="fleetcraft-clean-install-") as temporary:
        ...
        try:
            ...
            run([sys.executable, str(install_path / "scripts" / "fleetcraft-doctor.py")], env=env, cwd=install_path)
            guarded = run(
                [sys.executable, str(install_path / "hooks" / "fleet-delegation-guard.py")],
